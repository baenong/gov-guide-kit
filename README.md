...
터미널에서 내 저장소 루트(커밋되지 않은 변경사항이 없는 상태)에서 위 명령을 실행하면 됩니다. 처음 실행할 때뿐 아니라 템플릿이 다시 업데이트될 때마다 재실행하면 됩니다. 실행 중 충돌이 표시되면, 그건 대부분 내가 직접 고친 에디터 코드와 템플릿 쪽 수정이 같은 부분을 건드린 경우이니 직접 어느 쪽을 남길지 확인 후 커밋하면 됩니다.
